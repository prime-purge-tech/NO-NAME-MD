...
export default async function unmuteCommand(message, client) {
  ...
  if (!remoteJid.endsWith("@g.us")) {
    return await sendThemedMessage(client, remoteJid, photo, formatThemedMessage(remoteJid, "UNMUTE", ["❌ Uniquement dans un groupe !"]), [], message);
  }
  ...
  try {
    ...
    if (!estAdmin) {
      return await sendThemedMessage(client, remoteJid, photo, formatThemedMessage(remoteJid, "UNMUTE", ["❌ Réservé aux *admins du groupe* !"]), [], message);
    }

    await client.groupSettingUpdate(remoteJid, "not_announcement");

    await sendThemedMessage(client, remoteJid, photo, formatThemedMessage(remoteJid, "UNMUTE", [
      "✅ Groupe *démuté* !",
    ...
    console.error("Erreur unmuteCommand:", err.message);
    await sendThemedMessage(client, remoteJid, photo, formatThemedMessage(remoteJid, "UNMUTE", [
